...
static void inline csw_load_regs(struct vcpu *v, struct cpu_user_regs *stack_regs)
{
...
static void __context_switch(void)
{
    ...
    seg_desc_t           *gdt;
    bool                  need_full_gdt_n, switch_gdt;
    ...
    switch_gdt = (p->vcpu_id != n->vcpu_id) || !per_cpu(full_gdt_loaded, cpu);

    if ( need_full_gdt_n )
        write_full_gdt_ptes(gdt, n);

    if ( per_cpu(full_gdt_loaded, cpu) && (switch_gdt || !need_full_gdt_n ) )
    ...
    if ( need_full_gdt_n && (switch_gdt || !per_cpu(full_gdt_loaded, cpu)) )
        load_full_gdt(n, cpu);
